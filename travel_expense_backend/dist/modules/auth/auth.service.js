...
let AuthService = class AuthService {
    ...
    async ssoLogin(ssoToken) {
        if (!ssoToken || ssoToken === 'invalid_token') {
            throw new common_1.UnauthorizedException('Invalid or expired SSO token.');
        }
        let targetEmail = 'emp1@enterprise.com';
        ...
        if (!user) {
            throw new common_1.UnauthorizedException('SSO authenticated user not registered in local database.');
        }
